...
export default async function CreatePostPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const [{ data: categories }, {data: tags}] = await Promise.all([
    ...
    supabase.from("tags").select("id,name").order("created_at", {ascending: false}).limit(50),]);
  ...
  return (
    <div className="max-w-3xl">
      ...
      <form action={createPostAction} className="card mt-8 space-y-4">
        ...
        <input className="input" name="tags" placeholder="Теги через запятую: дизайн, арт, ui" list="suggested-tags"/>
        <datalist id="suggested-tags">{tags?.map((t: any) => <option key={t.id} value={t.name} />)}</datalist>
