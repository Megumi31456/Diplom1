...
export async function createPostAction(formData: FormData) {
  ...
  // Синхронизируем теги со справочником
  if (tags.length > 0) {
    await supabase
      .from("tags")
      .upsert(tags.map(name => ({ name, is_active: true })), { onConflict: "name" });
  }
  revalidatePath("/app");
...
export async function updatePostAction(formData: FormData) {
  ...
  // Обновляем теги: удаляем старые связи, создаём/связываем новые
  ...
  revalidatePath("/moderator");
  redirectWithParams(`/app/post/${post_id}`, {
    message: status === "published" ? "Публикация опубликована" : "Публикация сохранена как черновик",
  });
}
